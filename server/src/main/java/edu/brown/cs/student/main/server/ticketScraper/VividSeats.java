...
public class VividSeats implements Scraper {
  ...
  private void resetCookies() {
    ...
    System.setProperty(
        ...
        "C:\\Users\\mclaw\\Downloads\\chromedriver-win64\\chromedriver-win64\\chromedriver.exe");
  ...
  /**
   * This function sets the price and seat for a ticket given its link. If we can't laod the website
   * than we simply don't change anything as a baseline price has already been added. Usually only
   * works for the first ticket in the list.
   *
   * @param t
   */
  private void setPriceAndSeat(Ticket t) {
    ...
    try {
      ...
      Element sectionDiv =
          listingPrice.selectFirst(
              "div[class=styles_box__QqP94 styles_align-center__2ETCO styles_display-flex__3lYqk]");
  ...
  /**
   * This is called to get the info given a query. It can set everything except for the seat. The
   * price might not be found or could be innacurate but so far it's been working pretty decently.
   *
   * @param query
   * @return
   */
  public List<Ticket> getInfoGivenQuery(String query) {
    ...
    try {
      ...
      for (int i = 0; i < numberOfTix; i++) {
        ...
        Ticket t =
            new Ticket(
                minPrice, date, name, link, time, location,
                "unknown"); // we see that it sets everything but the seat
        tickets.add(t);
      }
    } catch (Exception e) {
      System.out.println(e);
