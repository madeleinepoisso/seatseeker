...
/**
 * The SeatGeek class is a scraper that returns the best tickets given a query. The downside to this
 * scraper is that the seatgeek bot detection is pretty good, so I wasn't able to get the actual
 * seats, but it does get the price pretty reliably.
 */
public class SeatGeek implements Scraper {
  /**
   * This method returns a list of tickets that are sorted in the same way that they appear on the
   * websites
   *
  ...
  /**
   * This function sets all the info given a query. It does this by using selenium and waiting for
   * the page to load completely before parsing the html and setting the properties of the ticket.
   *
   * @param query
   * @return
   */
  public List<Ticket> getInfoGivenQuery(String query) {
    ...
    System.setProperty(
        ...
        "C:\\Users\\mclaw\\Downloads\\chromedriver-win64\\chromedriver-win64\\chromedriver.exe"); // tells selenium where the driver is in my computer
    ChromeOptions options = new ChromeOptions();
    options.addArguments("--headless"); // Run in headless mode
    options.addArguments(
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36"); // Set user-agent
    ...
    try {
      Thread.sleep(
          ThreadLocalRandom.current()
              .nextInt(
                  2000,
                  4000)); // sleeps for a time between 2 and 4s to ensure that the page loads fully.
